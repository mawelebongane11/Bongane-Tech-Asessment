...
This application analyzes community poll data from text files to find the winning choice and list all voters who selected it. It is optimized for high performance and professional evaluation.
...
TypeScript requires more initial boilerplate code because you have to define data structures (interfaces) before using them. While this takes a little more time upfront, it prevents bugs later on and improves overall code quality.

### 3. Why did you select this particular language to solve the problem?
TypeScript is the industry standard for modern web applications. It allows for a fast development cycle while ensuring high code quality through static analysis. It is the best choice for building a professional, production-ready tool.

### 4. Did you do anything to make the solution run faster?
Yes. I implemented an **O(N) linear complexity** algorithm. Instead of searching through the list multiple times (which would be slow for large files), the app uses a **Hash Map** to index users and tally votes in a single pass. This ensures the analysis is instant, even for massive datasets.

### 5. How is the list sorted?
The list of voters for the winning color is sorted alphabetically to ensure it is easy to navigate. It first sorts by **Surname**, and then by **First Name** if surnames are identical. This provides a deterministic and professional output.

## Technical Note: Remote Data (Using fetch)
While this app currently uses file uploads for the assessment, it is designed to be extensible. To pull data from a remote server, you can use the standard JavaScript `fetch()` function:

```javascript
async function loadRemotePoll() {
  const response = await fetch('https://api.vula.com/poll-data');
  const data = await response.json();
  // Process the data using the O(N) analytical engine...
}
```
